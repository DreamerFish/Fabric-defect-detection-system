<template>
  <div class="forget">
    <div class="nav">
      <div class="inner">
        <div class="logo" @click="$router.push({ name: 'login' })">
          <span>海创-臂上眼</span>
        </div>
    ...
    <div class="findDiv">
      <div class="find">
        ...
        <div class="info" v-show="active == 0">
          ...
          <div class="inputrow">
            <el-button @click="toTwo" type="success">下一步</el-button>
          </div>
          <div class="inputrow">
            <el-button @click="$router.push({ name: 'login' })" type="danger"
              >已有账号，返回登录</el-button
            >
          </div>
          <div class="right">
        ...
        <div class="info" v-show="active == 1">
          ...
          <div class="inputrow">
            <span class="name">密保答案：</span
            ><el-input v-model="answer" placeholder="请输入问题答案"></el-input>
          </div>
          <div class="inputrow">
            <el-button @click="toThree">下一步</el-button>
          </div>
        </div>
        <div class="info" v-show="active == 2">
          ...
          <div class="inputrow">
            <el-button @click="reset">下一步</el-button>
          </div>
        </div>
        <div class="info" v-show="active == 4">
          <div class="inputrow">
            <span class="registerSuccess"
              ><i class="el-icon-success"></i>恭喜您，修改密码成功</span
            >
          </div>
          <div class="inputrow">
...
export default {
  ...
  methods: {
    ...
    async toThree() {
      ...
      var data = { username, question, answer };

      if (answer) {
        var result = await CHECKANSWER({ username, question, answer });
        console.log("密保问题", result);
        if (result.status == 0) {
